"""
Worker registry — tracks the OpenClaw host + screen-share publisher machine.
"""
import logging
import uuid
...
def register(label: str, machine_name: str, gateway_url: str, openclaw_mode: str = "openclaw") -> dict:
    row = {
        "id": str(uuid.uuid4()),
        "label": label,
...
def set_screen_ready(worker_id: str) -> dict:
    result = (
        get_supabase().table(TABLE)
...
def get_worker(worker_id: str) -> Optional[dict]:
    result = (
        get_supabase().table(TABLE).select("*").eq("id", worker_id).maybe_single().execute()
    )
    return result.data
